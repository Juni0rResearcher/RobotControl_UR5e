...
# Заданные в задании коэффициенты
KP = 100.0
KD = 20.0

# Загружаем модель Pinocchio
...
def inverse_dynamics_trajectory_controller(q: np.ndarray, dq: np.ndarray, t: float) -> np.ndarray:
    """
    Inverse Dynamics Controller для отслеживания траектории.
    Траектория — многотональная синусоида с малыми амплитудами вокруг удобной средней позы.
    """
...
def plot_results():
    ...
    Path("logs/plots/ID_trajectory_uncertain").mkdir(parents=True, exist_ok=True)
    ...
    plt.tight_layout()
    plt.suptitle('Inverse Dynamics: Position Tracking Errors (Safe Trajectory)', fontsize=16, y=0.98)
    plt.savefig('logs/plots/ID_trajectory_uncertain/01_position_errors.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    for i in range(6):
        ...
        plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.suptitle('Inverse Dynamics: Velocity Tracking Errors', fontsize=16, y=0.98)
    plt.savefig('logs/plots/ID_trajectory_uncertain/02_velocity_errors.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('logs/plots/ID_trajectory_uncertain/03_tracking.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('logs/plots/ID_trajectory_uncertain/04_torques.png', dpi=200, bbox_inches='tight')
    plt.close()

    # Статистика
    print("\n" + "="*70)
    print("INVERSE DYNAMICS - Uncertain")
    print("="*70)
...
def main():
    ...
    print("\nЗапуск Inverse Dynamics Controller c возмущением системы")
    ...
    sim = Simulator(
        ...
        record_video=True,
        video_path="logs/videos/07_inverse_dynamics_uncertain.mp4",
        width=1920,
        height=1080
    )

    sim.set_controller(inverse_dynamics_trajectory_controller)

    #Установка коэффициентов демпфирования суставов
    damping = np.array([0.5, 0.5, 0.5, 0.1, 0.1, 0.1]) # H/(рад/с)
    sim.set_joint_damping(damping)
    # Установка коэффициентов трения суставов
    friction = np.array([1.5, 0.5, 0.5, 0.1, 0.1, 0.1]) # HM
    sim.set_joint_friction (friction)
    # Модификация массы конца эффектора
    sim. modify_body_properties("end_effector" , mass=4)

    sim.run(time_limit=15.0)

    print("Симуляция завершена без падений.")
    print("Видео: logs/videos/07_inverse_dynamics_uncertain.mp4")
