...
def plot_results():
    ...
    Path("logs/plots/ID").mkdir(parents=True, exist_ok=True)
    ...
    plt.savefig('logs/plots/ID/01_position_errors.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('logs/plots/ID/02_velocity_errors.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('logs/plots/ID/03_tracking.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('logs/plots/ID/04_torques.png', dpi=200, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("\nЗапуск Inverse Dynamics Controller...")
    ...
    # 15 секунд — достаточно для нескольких периодов
    sim.run(time_limit=15.0)

    print("Симуляция завершена без падений.")
    print("Видео: logs/videos/06_inverse_dynamics.mp4")
